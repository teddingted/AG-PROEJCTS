...
OUT_FILE = "optimization_compact_v4.csv"
FLOWS = [600, 700, 800] 
...
class HysysEngine:
    ...
    def set_inputs_safe(self, flow, p_bar, t_deg):
        try:
            ...
            self.s10.MassFlow.Value = flow / 3600.0
            # Gradual P change if large
            if abs(self.s1.Pressure.Value - p_bar*100) > 100: # 1 bar diff
                self.s1.Pressure.Value = p_bar * 100.0
            ...
            # **SETTLE BUFFER**: Extra 1.0s MUST pass for Adjusts/Exchangers
            time.sleep(1.0) 
