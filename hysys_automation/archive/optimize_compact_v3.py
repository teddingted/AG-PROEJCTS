...
OUT_FILE = "optimization_compact_v3.csv"
FLOWS = [600, 700, 800] 
...
class HysysEngine:
    ...
    def recover_state(self, flow, p_bar):
        try:
            ...
            # Relaxed wait for recovery
            self.wait_stable(20, stable_time=2.0)
            
            for adj in self.adjs: adj.Reset()
            self.wait_stable(10, stable_time=2.0)
            
            return self.is_healthy()
        except: return False

    def wait_stable(self, timeout=30, stable_time=2.0):
        """Increased stability requirement to 2.0s"""
        time.sleep(0.5)
        start = time.time()
    ...
    def set_inputs_safe(self, flow, p_bar, t_deg):
        try:
            ...
            if abs(self.s1.Pressure.Value - p_bar*100) > 10:
                self.s1.Pressure.Value = p_bar * 100.0
                self.wait_stable(10, 2.0)
            
            self.adj4.TargetValue.Value = t_deg
            
            if not self.wait_stable(25, 2.0): return False
    ...
    for p in pressures:
        ...
        for t in range(-90, -125, -2):
            ...
            if 0.5 <= app <= 3.5:
                ...
                # Print small debug info periodically or just dot
                # print(f".({app:.1f})", end="") 
                # Keeping it concise but informative if stuck
                print(".", end="")
            elif app > 3.5:
                print("W", end="")
