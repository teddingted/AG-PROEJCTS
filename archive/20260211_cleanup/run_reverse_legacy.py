...
"""
HYSYS OPTIMIZER FINAL - REVERSE RUN (1300 -> 500 kg/h)
- Exact replica of hysys_optimizer_final.py logic
- Flows reversed
- Self-contained (No external dependency for popup)
"""

# --- Configuration ---
SIM_FILE = "Efficiency Increase_1.5 t_2 min. temp without heat recovery.hsc"
FOLDER = "hysys_automation"
OUT_FILE = "optimization_reverse_legacy_result.csv"

# REVERSE FLOWS
FLOWS = [1300, 1200, 1100, 1000, 900, 800, 700, 600, 500]
...
def optimize_flow(optimizer, flow):
    ...
    pressures = [round(center_p + i*0.1, 1) for i in range(-1, 2)] # Reduced scan for speed (-1 to +1)
    best = None
    for p in pressures:
        ...
        for t in range(-90, -125, -2):
            ...
            if 0.5 <= app <= 3.5:
                ...
                break
            elif app < 0.5 or app > 100: print(".", end="") 
            elif app > 3.5: print("W", end="") 
...
def main():
    ...
    with open('hysys_automation/' + OUT_FILE, 'w', newline='') as csvf:
        csv.writer(csvf).writerow(["MassFlow", "P_bar", "T_C", "MinApp_C", "Power_kW"])

    for f in FLOWS:
        res = optimize_flow(optimizer, f)
        if res:
             with open('hysys_automation/' + OUT_FILE, 'a', newline='') as csvf:
